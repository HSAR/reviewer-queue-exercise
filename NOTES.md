...
- Seed data is loaded into Postgres through Docker SQL init scripts.
- There is a persistence layer now, but the current implementation still assumes a single local Docker environment.
...
The backend owns the workflow rules in `backend/app/main.py`, with review items stored in Postgres and queried using raw SQL.
...
- I used raw SQL for the persistence layer to keep the implementation lightweight and easy to inspect.
- I used Docker development servers and SQL init scripts for simple local startup instead of a fuller migration toolchain.
- I skipped authentication, richer audit logs, pagination, and true multi-user conflict handling.

## Improvements With More Time

- Add an action history table for reviewer decisions.
